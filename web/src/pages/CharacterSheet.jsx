...
// Reusable character sheet, usable both as a standalone page and embedded (e.g. on the game table).
export default function CharacterSheet({ characterId, character: characterProp, backTo, onSaved, allowEdit = true }){
  const [char, setChar] = useState(characterProp || null)
  ...
  useEffect(()=>{
    if(characterProp){ setChar(characterProp); return }
    ...
    <div className="sheet print-friendly">
      <div className="sheet-top">
        <div className="left-top">
          <h1 className="char-name">
            {isEditing ? <input value={form.name||''} onChange={e=>setForm(f=>({...f, name:e.target.value}))} /> : (char.name || 'Unnamed')}
            {!isEditing && <span className="char-menu" ref={menuRef}>
              <button type="button" className="char-menu-toggle" onClick={()=>setMenuOpen(current => !current)} aria-haspopup="true" aria-expanded={menuOpen} aria-label="Character sheet options">⚙</button>
              {menuOpen && <div className="char-menu-dropdown" role="menu">
                <button type="button" role="menuitem" onClick={()=>{ setMenuOpen(false); window.print() }}>Print Sheet</button>
                {allowEdit && <button type="button" role="menuitem" onClick={()=>{ setMenuOpen(false); setIsEditing(true) }}>Edit</button>}
              </div>}
            </span>}
          </h1>
          <div className="submeta">
            <span>{isEditing ? <input placeholder="Race" value={form.race||''} onChange={e=>setForm(f=>({...f, race:e.target.value}))} /> : (char.race || 'Unknown Race')}</span>
            <span>{isEditing ? <input placeholder="Subrace" value={form.subrace||''} onChange={e=>setForm(f=>({...f, subrace:e.target.value}))} /> : ('- ' + char.subrace || '')}</span>
            <span>{isEditing ? <input placeholder="Background" value={form.background||''} onChange={e=>setForm(f=>({...f, background:e.target.value}))} /> : ('- ' + char.background || '')}</span>
          </div>
          <div className="small-meta">Player: {isEditing ? <input value={form.player||''} onChange={e=>setForm(f=>({...f, player:e.target.value}))} /> : (char.player || 'Unknown Player')} {!isEditing && '• '}Alignment: {isEditing ? <input value={form.alignment||''} onChange={e=>setForm(f=>({...f, alignment:e.target.value}))} /> : (char.alignment || 'Unknown Alignment')}</div>
        </div>
        ...
        <div className="right-top">
          ...
          <div className="block">
            ...
            <div className="value">{isEditing ? <input type="number" value={form.hitPoints?.temporary||0} onChange={e=>setForm(f=>({...f, hitPoints:{...(f.hitPoints||{}), temporary: Number(e.target.value)}}))} style={{width:64}} /> : (char.hitPoints?.temporary || 0)}</div>
          </div>
        </div>
      </div>
      ...
      <footer className="sheet-footer">
        <div className="actions">
          ...
          {backTo && <Link to={backTo}>← Back</Link>}
        </div>
      </footer>
    </div>
  )
