  ...
  return <div className="game-layout">
    ...
    <div className="game-layout-content game-players-page">
      ...
      <section className="party-section"><div className="section-heading"><h2>Party roster</h2><strong className="roster-count">{game.members.length} players</strong></div>
        {game.members.length === 0 ? <p className="empty-roster">No players have joined yet. Share the join code to gather the party.</p> : <div className="party-grid">{game.members.map(member => <article className="party-member" key={member.user?._id || member.user}><span className="character-avatar">{(member.character?.name || member.user?.name || member.user?.email)?.charAt(0).toUpperCase()}</span><div><h3>{member.character?.name || 'Character pending'}</h3><p>{member.character?.classes?.map(characterClass => characterClass.name).join(' / ') || 'No class selected'} · Level {member.character?.level || '-'}</p><span>{member.user?.name || member.user?.email}</span></div></article>)}</div>}
      </section>
